...
document.getElementById('currencyForm').addEventListener('submit', async function (event) {
  ...
  console.log("💾 Saving preferred currencies:", selectedCurrencies);

  try {
    await chrome.storage.sync.set({ preferredCurrencies: selectedCurrencies });
    alert('✅ Preferences saved!');
    
    // Optional: Notify background script to refresh menu
...
window.addEventListener('load', async () => {
  try {
    ...
    console.log("📂 Loaded saved currencies:", saved);
